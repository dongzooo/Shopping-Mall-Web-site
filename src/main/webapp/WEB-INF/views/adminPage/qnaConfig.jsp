...
<head>
    ...
    <link rel="stylesheet" href="/resources/css/adminPage/qnaConfig.css">
...
<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
        <section class="content-board">
            ...
            <div class="board_list_menu">
                <a href="#QNA_category" class="QNA_category">QNA 카테고리</a>
            ...
            <div class="board-category-group">
                <div class="unreg_board_category">
                	 <c:forEach var="item" items="${cateTempList}" >
						<p class="cate_list unreg_list" id="unreg${item.category_num}"> ${item.category_name} </p>
                    </c:forEach>
                    <!-- 
                    <c:forEach var="item" items="${unCatelist}" varStatus="status">
						<p class="cate_list unreg_list" id="unreg${status.index}"> ${item} </p>
                    </c:forEach> -->
                </div>
                ...
                <div class="reg_board_category">
                    <c:forEach var="item" items="${cateList}">
						<p class="cate_list reg_list" id="reg${item.qna_cate_num}"> ${item.qna_cate_name} </p>
                    </c:forEach>
                </div>
            </div>
            
            <!-- 등록 카테고리 값 저장 및 전송용 폼 -->
            <form class="hdn_form" action="/adminPage/qnaDeReg" method="post">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
            ...
            <div class="board_list_btn">
            	<button type="button" class="cate_btn add" onclick="openQnaAdd()">추가</button>
        ...
        $(document).ajaxSend(function(e, xhr, options) {
    		xhr.setRequestHeader( "${_csrf.headerName}", "${_csrf.token}" );
       		...
       		// 카테고리 추가 팝업
       		function openQnaAdd(){
       			let url = "/adminPage/qnaAdd";
       			
       			open(url, "QnaAdd", "toolbar=no, location=no, status=no, menubar=no, resizable=no, width=450, height=200");
       		}
       		...
       		$(document).on("click",".unreg_list", function(){
       			...
       			console.log("unreg_list : " +$(this).text()+ $(this).attr('id'));
       		});
